'use client';

import { useState } from 'react';
import { useTags, useTagCashFlow } from '@/hooks/dashboard/useTags';
import { ChartCard } from '@/components/dashboard/ChartCard';
import {
  ComposedChart, Bar, Line, XAxis, YAxis, CartesianGrid,
  Tooltip, Legend, ResponsiveContainer,
} from 'recharts';
import { format, parseISO } from 'date-fns';
import { X } from 'lucide-react';

const CAD = new Intl.NumberFormat('en-CA', { style: 'currency', currency: 'CAD', maximumFractionDigits: 0 });

function formatPeriod(period: string) {
  try {
    const d = period.length === 7 ? parseISO(period + '-01') : parseISO(period);
    return period.length === 7 ? format(d, 'MMM yy') : format(d, 'MMM d');
  } catch { return period; }
}

function DrillDownTooltip({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-[var(--bg-card)] border border-[color:var(--color-azure)] rounded-[10px] px-3 py-2.5 shadow-card text-[13px] font-sans min-w-[160px]">
      <p className="text-[12px] text-[var(--text-muted)] mb-2">{label}</p>
      {payload.map((p) => (
        <div key={p.name} className="flex justify-between gap-4">
          <span style={{ color: p.color }}>{p.name}</span>
          <span className="font-medium text-[var(--text-primary)]">{CAD.format(p.value)}</span>
        </div>
      ))}
    </div>
  );
}

function TagDrillDown({ tag, onClose }: { tag: string; onClose: () => void }) {
  const { data, isLoading, error } = useTagCashFlow(tag);
  const chartData = (data ?? []).map((d) => ({
    period: formatPeriod(d.period),
    Inflow: d.totalInflow,
    Outflow: Math.abs(d.totalOutflow),
    'Net Flow': d.netFlow,
  }));

  return (
    <ChartCard
      title={`Tag: ${tag}`}
      subtitle="Cash flow for selected tag"
      isLoading={isLoading}
      error={error}
      isEmpty={!isLoading && !error && chartData.length === 0}
      height={240}
    >
      <div className="relative">
        <button
          type="button"
          onClick={onClose}
          className="absolute top-0 right-0 p-1 text-[var(--text-muted)] hover:text-[var(--text-primary)] z-10"
        >
          <X className="size-4" />
        </button>
      </div>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={chartData} margin={{ top: 4, right: 24, bottom: 0, left: 8 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(58,180,232,0.18)" vertical={false} />
          <XAxis dataKey="period" tick={{ fontSize: 11, fontFamily: 'var(--font-inter)', fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} />
          <YAxis tickFormatter={(v) => CAD.format(v)} tick={{ fontSize: 11, fontFamily: 'var(--font-inter)', fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} width={72} />
          <Tooltip content={<DrillDownTooltip />} />
          <Legend wrapperStyle={{ fontSize: 12, fontFamily: 'var(--font-inter)', paddingTop: 8 }} />
          <Bar dataKey="Inflow"   fill="#3ab4e8" radius={[3, 3, 0, 0]} maxBarSize={32} />
          <Bar dataKey="Outflow"  fill="#e8593c" radius={[3, 3, 0, 0]} maxBarSize={32} />
          <Line dataKey="Net Flow" stroke="#1a6fa3" strokeWidth={2} dot={false} />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
}

export function TagsChart() {
  const { data, isLoading, error } = useTags();
  const [selectedTag, setSelectedTag] = useState<string | null>(null);

  return (
    <div className="space-y-4">
      <ChartCard
        title="Spending by Tag"
        subtitle="Click a tag to drill into its cash flow"
        isLoading={isLoading}
        error={error}
        isEmpty={!isLoading && !error && (data ?? []).length === 0}
        height={180}
      >
        <div className="h-full overflow-y-auto pr-1">
          <div className="flex flex-wrap gap-2 py-1">
            {(data ?? []).map((t) => (
              <button
                key={t.tag}
                type="button"
                onClick={() => setSelectedTag(selectedTag === t.tag ? null : t.tag)}
                className={[
                  'px-3 py-1.5 rounded-[10px] text-[12px] font-sans font-medium border transition-colors',
                  selectedTag === t.tag
                    ? 'bg-[var(--color-azure)] text-white border-[color:var(--color-azure)]'
                    : 'bg-[var(--muted)] text-[var(--text-secondary)] border-[color:var(--border)] hover:border-[color:var(--color-azure)] hover:text-[var(--color-deep)]',
                ].join(' ')}
              >
                {t.tag}
                <span className="ml-1.5 opacity-70">{CAD.format(Math.abs(t.totalSpend))}</span>
              </button>
            ))}
          </div>
        </div>
      </ChartCard>

      {selectedTag && (
        <TagDrillDown tag={selectedTag} onClose={() => setSelectedTag(null)} />
      )}
    </div>
  );
}
